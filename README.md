...
- **Use Gemini Flash as an external workhorse** - Codex and Claude brains can proactively route bounded cheap labour through Switchboard/`agy`, without treating Flash as a native child or authoritative brain.
...
3. **External workhorse lane** — Codex and Claude brains should proactively consider newest live Flash High through Agent Switchboard/`agy` for bounded search/read/extraction/summaries/drafting, low-risk implementation/tests from an approved plan, and independent parallel packages. Flash is not a native child agent. If `agy`/Flash is missing, quota-limited, times out, mismatches, or fails, use the host's native cheap reader/workhorse and record the fallback. Flash and native workers may run concurrently only for independent stages: parallel reads are allowed; writes stay serial unless demonstrably isolated. The brain reviews evidence/diffs and decides.
...
Use `route_agent_task` with `target_agent: "antigravity"`, `surface: "cli"`, `target_model: "gemini flash"`, and `effort: "high"`. Set `mode: "plan"` for read-only work, or `mode: "accept-edits"` only for an approved isolated implementation package.
