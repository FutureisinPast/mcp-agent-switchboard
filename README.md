...
- **Consult flagships without dumping context** - one `consult_decision` call sends only a structured, byte-capped brief to Astra and/or Fable, with effort scaled by complexity.
...
3. **Flagship decision lane** — `consult_decision` accepts one structured brief and maps `bounded` / `architecture` / `critical` to `high` / `xhigh` / `max`. Sol-or-lower Codex hosts consult Astra and add Fable for architecture/critical work; Astra consults Fable. Opus-or-lower Claude hosts consult Fable and add Astra for architecture/critical work; Fable consults Astra. Gemini hosts consult both. The broker prevents self-consultation, caps the brief at 12,000 UTF-8 bytes / about 3,000 tokens, and returns quota/unavailability notices for the final handoff instead of substituting Flash for flagship judgment.
