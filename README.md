...
- **Codex hook enforcement is now activated and diagnosed, not merely written to disk.** Installation safely adds the top-level `hooks` reference required by Codex, preserves existing configuration, refuses to replace a different user-owned hook file, and `doctor` reports missing events, activation mismatches, and whether the current session has produced runtime evidence. The deterministic gate harness now covers important read-only decision work as well as mutating work.
