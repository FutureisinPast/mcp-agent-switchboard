...
- For important architecture, compatibility, or hard decisions, call `consult_decision` with one bounded structured brief. A Codex Sol-or-lower host uses Astra for bounded decisions and Astra plus Fable (Claude chain `{claude_chain}`) for architecture/critical work; an Astra host uses Fable. A Claude Opus-or-lower host uses Fable for bounded decisions and Fable plus Astra (Codex frontier `{frontier}`) for architecture/critical work; a Fable host uses Astra. Gemini hosts use both Astra and Fable.
- `consult_decision` maps `bounded`/`architecture`/`critical` to `high`/`xhigh`/`max`. Its brief contains one decision, at most three questions, four options, eight evidence refs, 4,000 excerpt bytes, 12,000 total UTF-8 bytes, and about 3,000 tokens. Send summaries, precise excerpts, and immutable refs -- never full files, articles, logs, or chat histories.
...
|---|---|---|---|
| Architecture, ambiguity, security/auth/payment/migration, irreversible ops, approval, hard diagnosis, flagship decision consult | brain | `consult_decision` with bounded brief (Astra/Fable high, xhigh, or max) | -- |
