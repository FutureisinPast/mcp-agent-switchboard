...
- Codex, Claude, and Gemini brains use the newest live Antigravity Gemini Flash High through Agent Switchboard as the default fast, cheap external workhorse for bounded search, reading, extraction, summaries, drafting/writing, independent parallel read-only packages, and (once containment is enabled) isolated low-risk implementation/tests from an approved plan.
...
- Native cheap roles: a Codex brain uses the managed `explorer` (`{reader}`/low) and `worker` (`{workhorse}`/medium); a Claude brain uses managed `Explore` (`{claude_roles.get('reader') or 'haiku'}`) and `economy-worker` (`{claude_roles.get('workhorse') or 'sonnet'}`/medium). They remain the correct lane only for the stated exceptions above and for anything needing the host's own tools -- the fallback, not the first choice; record the fallback when one is used. The external Antigravity Flash lane is the default labour workhorse, not a native child agent, and never a frontier consultant or Astra/Fable decision consultant.
