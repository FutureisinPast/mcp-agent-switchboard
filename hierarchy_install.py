...
- For non-trivial planning or a hard issue, the brain must obtain one opposite-vendor maximum-effort consultation: a Codex brain uses Claude `{claude_chain}` with runtime attestation unless Claude is already latched unavailable for this main session; a Claude brain uses the live Codex frontier `{frontier}` at the highest available single-agent effort. Report any fallback.
- Claude availability is session-scoped and fail-closed. After the first explicit Claude quota, reachability, entitlement, subscription, access, or whole-family-unavailability failure in a main session, mark Claude unavailable for the rest of that session. Do not retry Fable, Opus, or any other Claude model, and do not automatically retry after a delay; reset the unavailable state only when a new main session begins.
- Capability tier outranks model version. Gemini Flash High is a useful, non-authoritative workhorse-level adviser; a higher version does not promote it above Sol/Fable or make its advice automatically authoritative. While Claude is latched unavailable, a Codex brain should request a second opinion from the newest live Antigravity Flash High, use it only as a degraded non-authoritative second opinion, label it degraded advisory fallback, and retain final judgment.
