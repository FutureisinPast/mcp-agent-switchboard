...
class HierarchyInstallTests(unittest.TestCase):
    ...
    def test_refresh_migrates_legacy_preserves_user_settings_and_is_idempotent(self):
        ...
        self.assertIn("same-vendor flagship consultation through its native child-agent mechanism", hierarchy_lower)
        self.assertIn("pass its compact structured outcome to `consult_decision`", hierarchy_lower)
        self.assertIn("must never replace this step with a nested same-vendor cli call", hierarchy_lower)
        self.assertIn("returns `needs_native_consultation` without contacting a provider", hierarchy_lower)
        self.assertIn("architecture/critical work adds only the opposite-vendor consultant", hierarchy_lower)
        ...
        self.assertIn("only codex and claude brains use the newest live antigravity gemini flash high", hierarchy_lower)
        self.assertIn("gemini, antigravity, and unknown hosts receive no automatic downward cost routing", hierarchy_lower)
        self.assertIn("gemini's upward astra/fable consultation", hierarchy_lower)
        self.assertIn("bounded reading, search, extraction, summaries, drafting/writing", hierarchy_lower)
        self.assertIn("light implementation and tests from an approved plan", hierarchy_lower)
        ...
        self.assertIn("codex brain uses the managed `explorer`", hierarchy_lower)
        self.assertIn("and `worker`", hierarchy_lower)
        self.assertIn("claude brain uses managed `explore`", hierarchy_lower)
        self.assertIn("and `economy-worker`", hierarchy_lower)
        self.assertIn("record the package-specific fallback", hierarchy_lower)
    ...
    def test_body_limits_automatic_downward_routing_and_defines_native_lanes(self):
        body = hierarchy_install.routing_rules_body(CODEX_ROLES, CLAUDE_ROLES).lower()
        self.assertIn("only codex and claude brains", body)
        self.assertIn("no automatic downward cost routing", body)
        self.assertIn("explicit routing", body)
        self.assertIn("upward astra/fable consultation", body)
        self.assertIn("choose reader for quick_check/research/search", body)
        self.assertIn("choose workhorse for implementation", body)
        self.assertIn("never prompt guesses or a silent flagship", body)
        self.assertIn("never auto-launch native work", body)
